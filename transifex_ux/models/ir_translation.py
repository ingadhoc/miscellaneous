...
class IrTranslation(models.Model):
    ...
    def _get_transifex_url(self):
        """ Overwrite complete original method in order to take into account
        another transifex urls in a specific order: first adhoc, second oca,
        third odoo.
        """
        ...
        alt_url = {
            'adhoc': get_param('transifex_ux.tx_adhoc_url'),
            'oca': get_param('transifex_ux.tx_oca_url'),
        }
        ...
        if not base_url or not tx_sections:
            ...
            for translation in self:
                ...
                if 'adhoc' in project:
                    base_url = alt_url.get('adhoc').rstrip('/')
                elif 'oca' in project:
                    base_url = alt_url.get('oca').rstrip('/')
