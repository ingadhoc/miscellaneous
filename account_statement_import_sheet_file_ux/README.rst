 ...
 * ``import_file_button`` is wrapped with ``*args, **kwargs``:
   ``account_statement_import_sheet_file_bg`` re-enters that same method from
   its background job with ``wizard_data=...`` and this module's override sits
   in front of it, so narrowing the signature makes every background import die
   with a ``TypeError`` and save nothing.
 * Inside a background job the failure is left as a ``UserError``: there is
   nobody there to click a redirect, and the job reports the message.
 * A failure raised as a ``UserError`` -- by this module or by the base one --
   is shown as it is, because it is already a sentence written for a person and
   already translated. Only what python itself raised gets a hint and is pushed
   behind "Technical detail", since that text is always in English. The type is
   read off ``__cause__``, which the base module keeps when it flattens the
