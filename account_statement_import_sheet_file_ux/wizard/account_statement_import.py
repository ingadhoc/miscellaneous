...
class AccountStatementImport(models.TransientModel):
    ...
    def _offer_the_preview(self):
        ...
        try:
            ...
            mapping = self.sheet_mapping_id
            if not mapping or self.env.context.get("bg_job"):
                # inside a background job there is nobody to click the button,
                # and the job reports a UserError, not a RedirectWarning
                raise
            message = str(error)
    ...
    def _sheet_mapping_error_message(self, mapping, message, cause=None):
        """The explanation first, the raw parser error last.

        A failure raised as a ``UserError`` -- by this module or by the base one
        -- is already a sentence written for a person, and already translated,
        so it is shown as it is. Only what python itself raised gets explained
        and pushed behind a label, because that text is a machine's and always
        in English.

        ``cause`` is the exception the sheet module wrapped: it flattens
        everything into the text of a ``UserError``, but keeps the original
        under ``__cause__``.
        """
        self.ensure_one()
    ...
    def _sheet_mapping_error_hint(self, mapping, message):
        ...
        The signals it reads are python's own wording, so they stay in English
        whatever the language of the user; a message written by a module is
        recognised by its type instead, upstream in
        ``_sheet_mapping_error_message``.
        """
        self.ensure_one()
